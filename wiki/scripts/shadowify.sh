...
### Content ###
for i in Ta_*; do
    sed -Ee '1,/^$/ { /^[[:space:]]*"?(due|ease|ivl|seen|lapses)"?:/d; }' "$i" >"../plugins/content/takeaways/$i" && echo "removing $i" && rm "$i"
done
...
find . -mindepth 1 -not -name 'Ta_*' -not -name 'Meta_*' -not -name '$__*' -exec mv {} ../plugins/content/book \;
